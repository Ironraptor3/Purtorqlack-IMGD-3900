...
const GOLD_TOUCH = ( function () {
    ...
    const update = function() {
        if (gameState === 2 && levelRunning) {
        ...
        else if (gameState === 1 && !levelRunning && path !== null
                && path.length > 0) {
            let nextPos = path.shift();
            for (let c = 0; c < INTERLUDES[level].length; ++c) {
                let character = CHARACTERS[INTERLUDES[level][c]];
                if (nextPos[0] === character.pos.x && nextPos[1] === character.pos.y) {
                    PS.statusText(character.text);
                    levelRunning = character.progress;
                    path = null;
                    break;
                }
            }
            if (path !== null) {
                ...
                PS.color(interludePos.x, interludePos.y, PS.COLOR_YELLOW);
    ...
    return {
        init : function() {
            ...
            const img = Array.from({length: GRID_SIZE_V*GRID_SIZE_H}, _ => 1);
